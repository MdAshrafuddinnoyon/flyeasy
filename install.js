...
async function runDatabaseSetup(pool) {
  const dumpFile = path.join(__dirname, 'flyeasy_full_dump.sql');
  if (fs.existsSync(dumpFile)) {
    log('🗄️  Importing full database dump...');
    const sql = fs.readFileSync(dumpFile, 'utf8');
    // We split by ';' but avoid splitting inside quotes.
    // The safest way is to use multipleStatements: true in the pool connection.
    try {
      await pool.query(sql);
    } catch (e) {
      console.log(YELLOW + '   Note: If you get syntax errors, ensure multipleStatements: true is set in createPool.' + RESET);
      throw e;
    }
  } else {
    throw new Error('flyeasy_full_dump.sql not found. Make sure it is in the same directory as install.js');
  }
}
